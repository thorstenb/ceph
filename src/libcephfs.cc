...
extern "C" int64_t ceph_telldir(class ceph_mount_info *cmount, struct ceph_dir_result *dirp)
{
  if (!cmount->is_mounted())
    return -ENOTCONN;
...
extern "C" void ceph_seekdir(class ceph_mount_info *cmount, struct ceph_dir_result *dirp, int64_t offset)
{
  if (!cmount->is_mounted())
    return;
...
extern "C" int64_t ceph_lseek(class ceph_mount_info *cmount, int fd,
...
extern "C" int ceph_ftruncate(class ceph_mount_info *cmount, int fd, int64_t size)
{
  if (!cmount->is_mounted())
    return -ENOTCONN;
...
extern "C" int ceph_ll_lookup_root(class ceph_mount_info *cmount,
                  Inode **parent)
{
...
extern "C" int ceph_ll_listxattr(class ceph_mount_info *cmount,
                              Inode *in, char *list,
