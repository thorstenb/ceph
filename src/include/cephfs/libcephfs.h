 ...
 *          inequality).
 */
int64_t ceph_telldir(class ceph_mount_info *cmount, struct ceph_dir_result *dirp);
 ...
 */
void ceph_seekdir(class ceph_mount_info *cmount, struct ceph_dir_result *dirp, int64_t offset);
 ...
 */
int ceph_readlink(class ceph_mount_info *cmount, const char *path, char *buf, int64_t size);
 ...
 * @returns 0 on success or a negative error code on failure.
 */
int ceph_truncate(class ceph_mount_info *cmount, const char *path, int64_t size);
 ...
 * @returns 0 on success or a negative error code on failure.
 */
int64_t ceph_lseek(class ceph_mount_info *cmount, int fd, int64_t offset, int whence);
/**
 ...
 */
int ceph_read(class ceph_mount_info *cmount, int fd, char *buf, int64_t size, int64_t offset);
 ...
 */
int ceph_write(class ceph_mount_info *cmount, int fd, const char *buf, int64_t size,
	       int64_t offset);
 ...
 * @returns 0 on success or a negative error code on failure.
 */
int ceph_ftruncate(class ceph_mount_info *cmount, int fd, int64_t size);
 ...
 * @return 0 on success or a negative error code on failure.
 */
int ceph_fallocate(class ceph_mount_info *cmount, int fd, int mode,
 ...
 */
int ceph_get_file_stripe_address(class ceph_mount_info *cmount, int fd, int64_t offset,
...
/* Low Level */
struct Inode *ceph_ll_get_inode(class ceph_mount_info *cmount,
				vinodeno_t vino);
 ...
 */
int ceph_ll_lookup_root(class ceph_mount_info *cmount,
                  Inode **parent);
int ceph_ll_lookup(class ceph_mount_info *cmount, struct Inode *parent,
		   const char *name, struct stat *attr,
		   Inode **out, int uid, int gid);
int ceph_ll_put(class ceph_mount_info *cmount, struct Inode *in);
int ceph_ll_forget(class ceph_mount_info *cmount, struct Inode *in,
		   int count);
int ceph_ll_walk(class ceph_mount_info *cmount, const char *name,
		 struct Inode **i,
		 struct stat *attr);
int ceph_ll_getattr(class ceph_mount_info *cmount, struct Inode *in,
		    struct stat *attr, int uid, int gid);
int ceph_ll_setattr(class ceph_mount_info *cmount, struct Inode *in,
		    struct stat *st, int mask, int uid, int gid);
int ceph_ll_open(class ceph_mount_info *cmount, struct Inode *in, int flags,
		 struct Fh **fh, int uid, int gid);
loff_t ceph_ll_lseek(class ceph_mount_info *cmount, struct Fh* filehandle,
		     loff_t offset, int whence);
int ceph_ll_read(class ceph_mount_info *cmount, struct Fh* filehandle,
		 int64_t off, uint64_t len, char* buf);
int ceph_ll_fsync(class ceph_mount_info *cmount, struct Fh *fh,
		  int syncdataonly);
int ceph_ll_write(class ceph_mount_info *cmount, struct Fh* filehandle,
		  int64_t off, uint64_t len, const char *data);
int64_t ceph_ll_readv(class ceph_mount_info *cmount, struct Fh *fh,
		      const struct iovec *iov, int iovcnt, int64_t off);
int64_t ceph_ll_writev(class ceph_mount_info *cmount, struct Fh *fh,
		       const struct iovec *iov, int iovcnt, int64_t off);
int ceph_ll_close(class ceph_mount_info *cmount, struct Fh* filehandle);
int ceph_ll_iclose(class ceph_mount_info *cmount, struct Inode *in, int mode);
/**
 ...
 */
int ceph_ll_getxattr(class ceph_mount_info *cmount, struct Inode *in,
		     const char *name, void *value, size_t size, int uid,
		     int gid);
int ceph_ll_setxattr(class ceph_mount_info *cmount, struct Inode *in,
		     const char *name, const void *value, size_t size,
		     int flags, int uid, int gid);
int ceph_ll_listxattr(class ceph_mount_info *cmount, struct Inode *in,
                      char *list, size_t buf_size, size_t *list_size, int uid, int gid);
int ceph_ll_removexattr(class ceph_mount_info *cmount, struct Inode *in,
			const char *name, int uid, int gid);
int ceph_ll_create(class ceph_mount_info *cmount, struct Inode *parent,
		   const char *name, mode_t mode, int flags,
		   struct stat *attr, struct Inode **out, Fh **fhp,
		   int uid, int gid);
int ceph_ll_mkdir(class ceph_mount_info *cmount, struct Inode *parent,
		  const char *name, mode_t mode, struct stat *attr,
		  Inode **out, int uid, int gid);
int ceph_ll_link(class ceph_mount_info *cmount, struct Inode *in,
		 struct Inode *newparrent, const char *name,
		 struct stat *attr, int uid, int gid);
int ceph_ll_truncate(class ceph_mount_info *cmount, struct Inode *in,
		     uint64_t length, int uid, int gid);
int ceph_ll_opendir(class ceph_mount_info *cmount, struct Inode *in,
		    struct ceph_dir_result **dirpp, int uid, int gid);
int ceph_ll_releasedir(class ceph_mount_info *cmount,
		       struct ceph_dir_result* dir);
int ceph_ll_rename(class ceph_mount_info *cmount, struct Inode *parent,
		   const char *name, struct Inode *newparent,
		   const char *newname, int uid, int gid);
int ceph_ll_unlink(class ceph_mount_info *cmount, struct Inode *in,
		   const char *name, int uid, int gid);
int ceph_ll_statfs(class ceph_mount_info *cmount, struct Inode *in,
		   struct statvfs *stbuf);
int ceph_ll_readlink(class ceph_mount_info *cmount, struct Inode *in,
		     char *buf, size_t bufsize, int uid, int gid);
int ceph_ll_symlink(class ceph_mount_info *cmount, struct Inode *parent,
		    const char *name, const char *value, struct stat *attr,
		    struct Inode **in, int uid, int gid);
int ceph_ll_rmdir(class ceph_mount_info *cmount, struct Inode *in,
		  const char *name, int uid, int gid);
uint32_t ceph_ll_stripe_unit(class ceph_mount_info *cmount,
			     struct Inode *in);
uint32_t ceph_ll_file_layout(class ceph_mount_info *cmount,
			     struct Inode *in,
			     struct ceph_file_layout *layout);
uint64_t ceph_ll_snap_seq(class ceph_mount_info *cmount,
			  struct Inode *in);
int ceph_ll_get_stripe_osd(class ceph_mount_info *cmount,
			   struct Inode *in,
			   uint64_t blockno,
			   struct ceph_file_layout* layout);
int ceph_ll_num_osds(class ceph_mount_info *cmount);
int ceph_ll_osdaddr(class ceph_mount_info *cmount,
		    int osd, uint32_t *addr);
uint64_t ceph_ll_get_internal_offset(class ceph_mount_info *cmount,
				     struct Inode *in, uint64_t blockno);
int ceph_ll_read_block(class ceph_mount_info *cmount,
		       struct Inode *in, uint64_t blockid,
		       char* bl, uint64_t offset, uint64_t length,
		       struct ceph_file_layout* layout);
int ceph_ll_write_block(class ceph_mount_info *cmount,
			struct Inode *in, uint64_t blockid,
			char* buf, uint64_t offset,
			uint64_t length, struct ceph_file_layout* layout,
			uint64_t snapseq, uint32_t sync);
int ceph_ll_commit_blocks(class ceph_mount_info *cmount,
