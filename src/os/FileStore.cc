...
int FileStore::mkfs()
{
  ...
  if (KeyValueDB::test_init(g_conf->osd_keyvaluedb, omap_dir)){
    dout(1) << g_conf->osd_keyvaluedb << " db exists/created" << dendl;
  } else {
    derr << "mkfs failed to create " << g_conf->osd_keyvaluedb << dendl;
    ret = -1;
    goto close_fsid_fd;
  }
  // journal?
...
int FileStore::mount() 
{
...

  {
    ...
    KeyValueDB * omap_store = KeyValueDB::create(g_ceph_context,
						 g_conf->osd_keyvaluedb,
						 omap_dir);
    if (omap_store == NULL)
    {
      derr << "Error creating " << g_conf->osd_keyvaluedb << dendl;
      ret = -1;
      goto close_current_fd;
    }
    ...
    if (omap_store->create_and_open(err)) {
      delete omap_store;
      derr << "Error initializing " << g_conf->osd_keyvaluedb << " : " << err.str() << dendl;
      ret = -1;
      goto close_current_fd;
    }
